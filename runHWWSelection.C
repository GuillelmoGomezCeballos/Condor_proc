// $Id: runHWWSelection.C,v 1.13 2013/10/21 19:34:03 dimatteo Exp $
...
void runHWWSelection(
	        ...
	        const char *book       = "cern/filefi/029",
		const char *dataset    = "r12a-smu-j13-v1", 
	        ...
	        int         nEvents    = 100)
{
  ...
  TString  myRootFile  = TString("histo_") + dataset;
  ...
  // Single Muon trigger
  TString smuTriggers[5] = {"HLT_IsoMu24_eta2p1_v11","HLT_IsoMu24_eta2p1_v12","HLT_IsoMu24_eta2p1_v13","HLT_IsoMu24_eta2p1_v14","HLT_IsoMu24_eta2p1_v15"};
  for (int i=0; i<5; i++) {
    hltModP->AddTrigger(smuTriggers[i],0,999999); //jet 85 , met 90
    hltModP->AddTrigger(TString("!"+smuTriggers[i]),0,999999); //jet 85 , met 90
  }
...
//   float minMet = 200;
  ...
  TString rootFileNameMake0 = TString(theOutputDir);
  rootFileNameMake0 += TString(myRootFile);
  rootFileNameMake0 += TString("_smurf0_") + TString(fileset);
  rootFileNameMake0 += TString("_") + TString("noskim");
  rootFileNameMake0 += TString("_TEMP.root");
  ...
  jetplusmettree->SetOutputName(rootFileNameMake0);

  TString rootFileNameMake1 = TString(theOutputDir);
  rootFileNameMake1 += TString(myRootFile);
  rootFileNameMake1 += TString("_smurf1_") + TString(fileset);
  rootFileNameMake1 += TString("_") + TString("noskim");
  rootFileNameMake1 += TString("_TEMP.root");
  ...
  dileptontree->SetOutputName(rootFileNameMake1);

  TString rootFileNameMake2 = TString(theOutputDir);
  rootFileNameMake2 += TString(myRootFile);
  rootFileNameMake2 += TString("_smurf2_") + TString(fileset);
  rootFileNameMake2 += TString("_") + TString("noskim");
  rootFileNameMake2 += TString("_TEMP.root");
  ...
  wlnutree->SetOutputName(rootFileNameMake2);
